...
# chromedriver vem do env (Linux no GH Actions); fallback local
service = Service(executable_path=os.environ.get('CHROMEDRIVER', './data/chromedriver'))
wd_opt = Options()
wd_opt.add_argument("--no-sandbox")
wd_opt.add_argument("--headless=new")
wd_opt.add_argument("--disable-gpu")
wd_opt.add_argument("--disable-software-rasterizer")
wd_opt.add_argument("--disable-dev-shm-usage")
wd_opt.add_argument("--disable-extensions")
wd_opt.add_argument("--disable-background-networking")
wd_opt.add_argument("--disable-sync")
wd_opt.add_argument("--no-first-run")
wd_opt.add_argument("--window-size=1280,1024")
...
if _chrome_bin:
    wd_opt.binary_location = _chrome_bin
elif os.path.exists('/opt/cft-chrome/linux64/chrome'):
    wd_opt.binary_location = '/opt/cft-chrome/linux64/chrome'
try:
    wd = webdriver.Chrome(service=service, options=wd_opt)
    print("[chrome] sessao iniciada ok (%s)" % wd.capabilities.get('browserVersion', '?'), flush=True)
except Exception as _e:
    _msg = str(_e)
    import traceback
    print("[chrome] FALHA AO INICIAR: %r" % (_e,), flush=True)
    print("[chrome] MSG: %s" % _msg[:1200], flush=True)
    traceback.print_exc()
    raise
